...
namespace OOTiZ
{
    public partial class Report : Form
    {
        ...
        //component initialization (this whole from)
        ...
        //collects all data across the DataSet. Added obj Plan as double
        public class Operation
        {
            ...
            public Operation
                (int ID, int? Number, string Name, double? Norma, string Razryad, string Koeff, double? Premia, double? zarp, string Primech, bool? Check, vypuskDataSet DataSet)
            {
                this.ID = ID;
                //////////////////////////
                if (Number != null)
        ...
        private class Detail
        {
            ...
            {
                if (this.Ceha == null & this.details == null & this.crews == null)
                {
                    //verifying all 'Ceha' to complete data
                    Ceha = new List<Ceh>();
                    ...
                    //verifying all 'crews' to complete data
                    crews = new List<Crew>();
                    foreach (DataRow Row in DataSet.Бригады.Rows)
                        if (Row.ItemArray[1].GetType() == typeof(string))
                            crews.Add(new Crew((int)Row.ItemArray[0], Row.ItemArray[1] as string));
                        else
                            crews.Add(new Crew((int)Row.ItemArray[0], ""));
                    //adding data to details List
                    this.details = new List<Uzel>();
                    
                    foreach (Ceh ceh in this.Ceha)
                        ceh.operations = new List<Operation>();
                    foreach (Crew crew in this.crews)
                        crew.operations = new List<Operation>();
                    //adding details array to 'Ceha'
                        ...
                        if ((int)Row.ItemArray[2] != this.ID) if (Row.ItemArray[2].GetType() == typeof(int) & Row.ItemArray[3].GetType() == typeof(int))
                            {
                                ...
                                //for (int i = 0; i < this.details[this.details.Count - 1].detail.Ceha.Count; i++)
                                //{
                                //    this.crews[i].Norma += this.details[this.details.Count - 1].detail.crews[i].Norma * this.details[this.details.Count - 1].Count;
                                //    this.crews[i].Rascenka += this.details[this.details.Count - 1].detail.crews[i].Rascenka * this.details[this.details.Count - 1].Count;
                                //}
                            }
                    }
        ...
        //Crew report
        private void GenerateReport_Brigadi(Detail detail, int level, int Count, List<object[]> CrewList)
        {
            string levelTabs = "";
            for (int i = 0; i < level; i++)
                levelTabs += "    | ";
            ...
            if (level == 0)
            {
                CrewList[CrewList.Count - 1][1] = detail.Name;
                CrewList[CrewList.Count - 1][0] = 'u';
            }
            else
            {
                CrewList[CrewList.Count - 1][1] = levelTabs.Remove(levelTabs.Length - 2) + '-' + detail.Name;
                CrewList[CrewList.Count - 1][4] = Count;
            }
            //the laziest way is to take coloumns from general report, modify & fill 'em
            if (level == 0 | Full.Checked)
                foreach (Detail.Crew crew in detail.crews)
                {
                    foreach (Operation operation in crew.operations)
                    {
                        CrewList.Add(new object[15 + DataSet.Бригады.Count]);
                        CrewList[CrewList.Count - 1][0] = null;
                        CrewList[CrewList.Count - 1][1] = "    | " + levelTabs;
                        CrewList[CrewList.Count - 1][2] = operation.Number;
                        CrewList[CrewList.Count - 1][3] = "    " + operation.Name;
                        CrewList[CrewList.Count - 1][4] = operation.Razryad;
                        CrewList[CrewList.Count - 1][5] = operation.Koeff;
                        CrewList[CrewList.Count - 1][6] = operation.Norma;
                        CrewList[CrewList.Count - 1][7] = operation.Rascenka;
                        CrewList[CrewList.Count - 1][8] = operation.Check;
                        CrewList[CrewList.Count - 1][9] = Math.Round((double)(operation.Norma * Count), 6);
                        CrewList[CrewList.Count - 1][10] = Math.Round((double)(operation.Rascenka * Count), 6);
                        CrewList[CrewList.Count - 1][11] = operation.Plan;
            ...
            if ((detail.details.Count > 0 | detail.OperationCount > 0) & (level == 0 | Full.Checked))
            {
                CrewList.Add(new object[13 + DataSet.Бригады.Count]);
                if (level == 0)
                {
                    CrewList[CrewList.Count - 1][0] = 'u';
                    CrewList[CrewList.Count - 1][1] = "Итого по " + detail.Name + ":";
                }
                else
                    CrewList[CrewList.Count - 1][1] = levelTabs + "Итого по " + detail.Name + ":";
            }
            CrewList[CrewList.Count - 1][7] = detail.Norma;
            CrewList[CrewList.Count - 1][8] = detail.Rascenka;
            CrewList[CrewList.Count - 1][9] = Math.Round((double)(detail.Norma * Count), 6);
            CrewList[CrewList.Count - 1][10] = Math.Round((double)(detail.Rascenka * Count), 6);
            for (int i = 0; i < DataSet.Бригады.Count; i++)
                CrewList[CrewList.Count - 1][12 + i] = Math.Round((double)(detail.crews[i].Norma * Count), 6);
            if (level == 0)
                CrewList.Add(new object[1]);
        }
        ...
        private void Generate_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton8.Checked)
            {
                ...
                progressBar1.Maximum = detailsSorted.Count;
                    ...
                    object[,] Table = new object[list.Count, 12]; //was 12
                    bool ContainsPodUzel = false;
                    for (int i = 0; i < list.Count; i++)
                    {
                        for (int j = 0; j < 6; j++) // was 6
                            Table[i, j] = list[i][j];
                        if (Table[i, 5] != null)
                    ...
                    report.Columns[6].NumberFormat = "0000,0";
                    ...
                    report.Columns[7].NumberFormat = "0000,0";
                    ...
                    report.Columns[11].NumberFormat = "0000,0";
                    ...
                    report.Columns[12].NumberFormat = "0000,0";
                    ...
                    report.Cells[list.Count + 7, 12].Formula = $"=SUM(L{list.Count + 5}:L{list.Count + 6})";
                       ...
                       Int32 normavirobatki;
                       string znachNV;
                       znachNV = comboBox2.Text;
                       normavirobatki = Int32.Parse(znachNV);
...
;
        ...
        // some other events not related to report
